...
- 등록 정보: 아파트명·주소 검색으로 위치를 선택하고 단지명을 확인하고 실거래에서 조회한 전용면적을 선택합니다. 법정동과 조회용 시군구 코드는 주소 검색 결과에서 자동으로 설정합니다. 전용면적은 소수점 아래를 버려 정수 부분이 같은 거래를 합산합니다(59.1·59.99㎡ → 59㎡, 84.1·84.99㎡ → 84㎡). 정수 부분이 다른 면적은 별도로 등록합니다.
...
2. 기존 환경변수 3개와 `SUPABASE_SECRET_KEY`, `CRON_SECRET`을 Vercel Project Settings → Environment Variables에 등록합니다. `SUPABASE_SECRET_KEY`는 Supabase Settings → API Keys의 서버 전용 secret key이며 브라우저에 노출하거나 `NEXT_PUBLIC_` 접두사를 붙이면 안 됩니다. `CRON_SECRET`은 16자 이상의 임의 문자열로 만듭니다.
3. 기존 프로젝트는 Supabase SQL Editor에서 `supabase/weekly-sync.sql`을 한 번 실행합니다. 새 프로젝트에서 최신 `supabase/schema.sql`을 실행했다면 생략합니다.
...
4. 카카오 동의항목에서 닉네임(`profile_nickname`)을 **선택 동의**로 설정합니다. 동의 목적은 “서비스 내 로그인한 사용자의 이름 표시”입니다. 이메일과 프로필 사진은 요청하지 않습니다.
5. Supabase Kakao 설정의 **Allow users without an email**을 켜고 저장합니다. 앱은 `queryParams.scope`로 닉네임만 요청합니다. `options.scopes`는 기본 범위에 추가되므로 이메일 제외에 사용하지 않습니다. 이메일 없는 카카오 계정은 기존 구글 계정과 자동으로 연결되지 않으므로 기존 기록은 이전 로그인 계정으로 확인합니다.
...
월별 중앙값·차액·누락 월·가격 기준 분리와 실거래 XML 파싱·취소 거래 필터를 테스트합니다. 사용자별 DB 격리는 실제 Supabase 프로젝트 구성 후 두 개의 테스트 계정으로 추가 확인해야 합니다.

### 아파트 주소 검색

카카오 우편번호 서비스로 아파트명·도로명·지번 주소를 검색합니다. 별도 API 키가 필요하지 않습니다. 선택 결과의 법정동 코드에서 조회용 지역코드를 추출하며, 기존 properties 스키마를 그대로 사용합니다. 닉네임 등 소셜 로그인 동의항목과는 별도 서비스입니다. 외부 스크립트와 검색 프레임을 사용하므로 네트워크 연결이 필요합니다.

### 전용면적 선택

단지 추가·수정에서 “단지 면적 불러오기”로 최근 12개월의 실거래 면적을 조회합니다. 이전 1년씩 최대 60개월까지 확장할 수 있습니다. 로그인과 기존 MOLIT_API_KEY를 사용하며 별도 키나 SQL 변경은 필요 없습니다. 단지명과 법정동을 정확히 일치시키고 해제 거래는 제외하며, 면적 선택과 거래 매칭 모두 소수점 아래를 버린 정수 부분을 사용하며, 거래 메모에는 원래 면적을 보존합니다. 거래가 없는 타입은 목록에 없을 수 있고 기존 소수점 등록 면적도 같은 정수 면적으로 조회됩니다. 이전 조회 기록에 정수 부분이 같은 다른 면적을 포함하려면 같은 기간을 다시 조회하세요. 월별 지역 자료는 서버에서 1시간 캐시하며 조회는 가격 기록을 저장하지 않습니다.
...
그래프는 `단지별 가격`과 `내 집과 비교` 탭으로 나뉩니다. 가격 탭은 실제 월별 기록만 표시하고 빈 달을 건너뛰어 선으로 연결합니다. 비교 탭은 보유 주택이나 관심단지의 가격이 바뀐 달마다 양쪽의 마지막 확인 가격을 사용합니다. 따라서 9월에 입력한 내 집 가격과 8월이 마지막 거래인 관심단지도 비교할 수 있으며, 내부 데이터에는 양쪽 가격의 기준 월을 함께 보관합니다.
