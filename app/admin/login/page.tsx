...
export default function AdminLoginPage() {
  ...
  async function localLogin() {
    ...
    setMessage("");
    try {
      const response = await fetch("/api/auth/local-admin", { method: "POST" }),
        result = await response.json();
  ...
  return (
    <main className="admin-page">
      <section className="panel auth-card">
        <h1>운영자 로그인</h1>
        <p>운영자 계정으로 문의함을 관리합니다.</p>
        {localSupabase && (
          <button className="social-button local-dev" disabled={busy} onClick={localLogin}>
            로컬 최고 관리자로 시작
          </button>
        )}
