...
export async function GET(request:Request){
 try{
  ...
  let query=admin.schema('admin').from('audit_events').select('id,actor_id,action,target_id,request_id,outcome,before_status,after_status,created_at').order('created_at',{ascending:false}).order('id',{ascending:false}).limit(50);
