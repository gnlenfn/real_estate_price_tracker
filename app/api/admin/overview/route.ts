...
export async function GET(request:Request){
 try{
  ...
  return Response.json({unansweredCount:unanswered.count||0,newTicketCount7d:newTickets.count||0,cron,recentFailures:allRuns.filter(run=>run.status==='failed'||run.status==='partial').slice(0,5).map(run=>({id:run.id,startedAt:run.started_at,failureCount:run.failure_count}))});
 }catch(error){return adminAccessResponse(error)??serverError('admin.overview',error,'운영 현황을 불러오지 못했습니다.');}
}
