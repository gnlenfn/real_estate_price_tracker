...
export async function GET(request:Request,{params}:{params:Promise<{ticketId:string}>}){
 try{
  const {ticketId}=await params,{admin}=await requireAdmin(request);
  const [ticketResult,messageResult,attachmentResult,noteResult]=await Promise.all([
   admin.from('support_tickets').select('id,user_id,category,title,body,status,screen,browser,created_at,last_activity_at,github_status,github_issue_number,github_issue_url').eq('id',ticketId).maybeSingle(),
   admin.from('support_messages').select('id,author_role,body,created_at').eq('ticket_id',ticketId).order('created_at'),
   admin.from('support_attachments').select('id,message_id,storage_path,file_name,mime_type,byte_size,created_at').eq('ticket_id',ticketId).order('created_at'),
   admin.from('support_internal_notes').select('id,author_id,body,created_at').eq('ticket_id',ticketId).order('created_at'),
  ]);
...
export async function PATCH(request:Request,{params}:{params:Promise<{ticketId:string}>}){
 try{
  const {ticketId}=await params,{user,admin}=await requireAdmin(request);
  ...
  const {data:before}=await admin.from('support_tickets').select('status').eq('id',ticketId).maybeSingle();
  if(body){const {error}=await admin.from('support_messages').insert({ticket_id:ticketId,author_id:user.id,author_role:'admin',body});if(error)return serverError('admin.support.reply',error,'답변을 저장하지 못했습니다.',500,{ticketId});}
  const next=status||(body?'answered':undefined);
  const {data,error}=await admin.from('support_tickets').update({...(next?{status:next}:{}),last_activity_at:now,updated_at:now}).eq('id',ticketId).select('id,status,last_activity_at').single();
