  ...
  try {
    ...
    const [ticketResult, messageResult, attachmentResult, noteResult] = await Promise.all([
      support
        .from("tickets")
        .select(
          "id,user_id,category,title,body,status,screen,browser,created_at,last_activity_at,github_status,github_issue_number,github_issue_url",
        )
        .eq("id", ticketId)
        .maybeSingle(),
      support
    ...
    if (messageResult.error || attachmentResult.error || noteResult.error)
      throw messageResult.error || attachmentResult.error || noteResult.error;
    const attachments = await Promise.all(
      (attachmentResult.data || []).map(async (attachment) => {
        const { data, error } = await admin.storage
          .from("support-attachments")
          .createSignedUrl(attachment.storage_path, 300);
        return { ...attachment, url: error ? null : data.signedUrl };
      }),
    );
    ...
    return Response.json({
      ticket: { ...ticketResult.data, nickname: profile?.nickname || "집 사용자" },
    ...
    return (
      ...
      serverError("admin.support.detail", error, "문의 내용을 불러오지 못했습니다.")
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const input = await request.json(),
      ...
      status = input.status as SupportStatus | undefined;
    ...
    if (!body && !status)
      return Response.json({ error: "변경할 내용을 입력해 주세요." }, { status: 400 });
    const now = new Date().toISOString(),
      requestId = crypto.randomUUID();
    const { data: before } = await support
      .from("tickets")
      .select("status")
      .eq("id", ticketId)
      .maybeSingle();
    if (body) {
      const { error } = await support
        .from("messages")
        .insert({ ticket_id: ticketId, author_id: user.id, author_role: "admin", body });
      if (error)
        return serverError("admin.support.reply", error, "답변을 저장하지 못했습니다.", 500, {
          ticketId,
        });
    }
    ...
    const { data, error } = await support
      .from("tickets")
      .update({ ...(next ? { status: next } : {}), last_activity_at: now, updated_at: now })
      .eq("id", ticketId)
      .select("id,status,last_activity_at")
      .single();
    if (error)
      return serverError("admin.support.update", error, "문의를 수정하지 못했습니다.", 500, {
        ticketId,
      });
    if (body)
    ...
    return (
      adminAccessResponse(error) ??
      serverError("admin.support.mutate", error, "문의 처리에 실패했습니다.")
    );
  }
}
