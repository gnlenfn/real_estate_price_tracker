...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await admin.schema("admin").rpc("list_support_tickets", {
      p_status: params.get("status") || "",
      p_category: params.get("category") || "",
      p_github: params.get("github") || "",
    ...
    return Response.json({
      tickets,
      nextCursor:
        tickets.length === 50 && last
          ? Buffer.from(JSON.stringify({ activity: last.last_activity_at, id: last.id })).toString(
              "base64url",
            )
          : null,
    });
    ...
    return (
      ...
      serverError("admin.support.list", error, "문의함을 불러오지 못했습니다.")
    );
  }
}
