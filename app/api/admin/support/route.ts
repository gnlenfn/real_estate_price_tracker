...
export async function GET(request:Request){
 try{
  ...
  const {data,error}=await admin.schema('admin').rpc('list_support_tickets',{p_status:params.get('status')||'',p_category:params.get('category')||'',p_github:params.get('github')||'',p_search:(params.get('search')||'').trim(),p_before_activity:cursor?.activity||null,p_before_id:cursor?.id||null,p_limit:50});
