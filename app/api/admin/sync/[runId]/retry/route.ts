...
export async function POST(request: Request, { params }: { params: Promise<{ runId: string }> }) {
  try {
    ...
    return Response.json({
      run: await runWeeklyTradeSync("manual", { retryOf: runId, actorId: user.id }),
    });
  } catch (error) {
    ...
    if (
      ...
      ["RETRY_NOT_ALLOWED", "RETRY_SCOPE_UNAVAILABLE"].includes(error.message)
    )
      return Response.json(
        {
          error:
            error.message === "RETRY_SCOPE_UNAVAILABLE"
              ? "실패 대상을 특정할 수 없어 전체 실행이 필요합니다."
              : "재시도할 실패 대상이 없습니다.",
        },
        { status: 409 },
      );
