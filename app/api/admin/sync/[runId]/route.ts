...
export async function GET(request:Request,{params}:{params:Promise<{runId:string}>}){
 try{
  ...
  const {data,error}=await admin.schema('admin').from('trade_sync_runs').select('id,trigger,status,started_at,finished_at,property_count,completed_count,saved_records,failure_count,failures,request_id,retry_of,actor_id,skipped_count,failure_scope').eq('id',runId).maybeSingle();
