...
export async function POST(request:Request){
 ...
 if((aptSeq!=null&&typeof aptSeq!=='string')||(jibunAddress!=null&&typeof jibunAddress!=='string')||String(aptSeq??'').length>100||String(jibunAddress??'').length>200)return Response.json({error:'단지 식별 정보를 확인해 주세요.'},{status:400});
 try{return Response.json(apartmentIdentity(await fetchDistrictTrades(district,month,key,true),{name,dong,aptSeq,jibunAddress}),{headers:{'Cache-Control':'no-store'}});}
 catch(error){return serverError('areas.lookup',error,'실거래 면적 목록을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.',502);}
}
