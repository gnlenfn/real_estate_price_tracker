...
export async function POST(request:Request){
 ...
 const authDb=createClient(url,anon,{db:{schema:'app'},global:{headers:{Authorization:authorization}},auth:{persistSession:false,autoRefreshToken:false}});
 ...
 const db=createClient(url,secret,{db:{schema:'app'},auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}}),support=db.schema('support');
 const since=new Date(Date.now()-60*60*1000).toISOString();
 const {count,error:countError}=await support.from('tickets').select('id',{count:'exact',head:true}).eq('user_id',user.id).gte('created_at',since);
 ...
 const {data:ticket,error:ticketError}=await support.from('tickets').insert({user_id:user.id,category,title,body,screen,browser,status:'open',github_status:'pending'}).select('id').single();
