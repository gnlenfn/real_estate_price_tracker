...
export async function POST(request: Request) {
  ...
  if (!url || !anon || !key)
    return Response.json({ error: "실거래 조회 기능을 사용할 수 없습니다." }, { status: 503 });
  ...
  try {
    ...
    if (!isTradeMonthAllowed(body.month))
      return Response.json({ error: "조회 월을 확인해 주세요." }, { status: 400 });
    const { data: property, error: pe } = await db
      .from("properties")
      .select("*")
      .eq("id", body.propertyId)
      .single();
    if (pe || !property)
      return Response.json({ error: "단지를 찾을 수 없습니다." }, { status: 404 });
    const records = await fetchTrades(property, body.month, key);
    const { error: saveError } = await db.rpc("replace_molit_month", {
      p_id: property.id,
      p_month: body.month,
      p_rows: records,
    });
    ...
    return serverError(
      "trades.fetch-and-save",
      error,
      "실거래가를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.",
      502,
    );
  }
}
