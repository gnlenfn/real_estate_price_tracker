...
export function AdminNav({active}:{active:Page}){
 ...
 return <header className="admin-nav"><Link className="admin-brand" href="/admin">집업 운영</Link><nav>{items.map(([id,label,href,icon])=><Link className={active===id?'active':''} href={href} key={id}>{icon}{label}</Link>)}</nav><button onClick={async()=>{clearAdminCache();await supabase?.auth.signOut({scope:'local'});window.location.assign('/admin/login');}}><LogOut size={17}/>로그아웃</button></header>;
}
