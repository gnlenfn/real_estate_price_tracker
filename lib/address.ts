...
export function propertyAddress(result:AddressResult){
 ...
 return {district:result.bcode.slice(0,5),dong,label:[result.sido,result.sigungu,dong].filter(Boolean).join(' '),name:result.buildingName.trim(),roadAddress:result.roadAddress.trim(),jibunAddress:result.jibunAddress.trim(),kakaoPlaceId:''};
}
