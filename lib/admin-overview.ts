...
export function weeklySchedule(now: Date) {
  const next = new Date(now),
    daysUntilSunday = (7 - now.getUTCDay()) % 7;
...
export function cronHealth(runs: Run[], now = new Date(), enabledAt?: string | null) {
  ...
  if (cron && new Date(cron.started_at) >= schedule.previous) {
    const health: CronHealth =
      cron.status === "running" &&
      now.getTime() - new Date(cron.started_at).getTime() > 15 * 60 * 1000
        ? "delayed"
        : (cron.status as CronHealth);
    return { health, lastRunId: cron.id, nextAt: schedule.next.toISOString() };
  }
  const activation = enabledAt ? new Date(enabledAt) : null,
    firstDue = activation && activation > schedule.previous ? schedule.next : schedule.previous;
  const health: CronHealth =
    now.getTime() > firstDue.getTime() + 60 * 60 * 1000 ? "delayed" : "unverified";
