...
export const labels: { [key in Kind]: string } = {
  trade: "실거래가",
  estimate: "직접 평가",
  asking: "호가",
};
...
export const money = (n: number | null | undefined) =>
  n == null ? "—" : `${(n / 10000).toLocaleString("ko-KR", { maximumFractionDigits: 2 })}억`;
...
function periodKey(date: string, interval: Interval) {
  if (interval === "month") return date.slice(0, 7);
  return isoDate(monday(new Date(`${date.slice(0, 10)}T00:00:00Z`)));
}
    ...
    interval === "month"
      ...
      : (() => {
          ...
          return Array.from({ length: periods }, (_, i) => {
            ...
            start.setUTCDate(start.getUTCDate() - (periods - 1 - i) * 7);
      ...
      row[p.id] = median(
        data.records
          .filter(
            (r) =>
              r.property_id === p.id &&
              r.kind === (p.id === baseId ? baseKind : kind) &&
              periodKey(r.date, interval) === period,
          )
          .map((r) => r.price),
      );
    return row;
    ...
    const prior = data.records
      .filter(
        (r) =>
          r.property_id === p.id &&
          r.kind === selectedKind &&
          periodKey(r.date, interval) < firstMonth,
      )
      .sort((a, b) => b.date.localeCompare(a.date));
    const priorMonth = prior[0] && periodKey(prior[0].date, interval);
    if (priorMonth) {
      const value = median(
        prior.filter((r) => periodKey(r.date, interval) === priorMonth).map((r) => r.price),
      );
      if (value != null) latest[p.id] = { value, month: priorMonth };
    }
  }
  for (const row of rows) {
    const changed = new Set<string>();
    for (const p of data.properties)
      if (typeof row[p.id] === "number") {
        latest[p.id] = { value: row[p.id] as number, month: String(row.month) };
        changed.add(p.id);
      }
    for (const p of data.properties) {
      const gapKey = `gap_${p.id}`;
      row[gapKey] = null;
      if (p.id === baseId || (!changed.has(baseId) && !changed.has(p.id))) continue;
      const home = latest[baseId],
        target = latest[p.id];
      if (home && target) {
        row[gapKey] = target.value - home.value;
        row[`gap_base_month_${p.id}`] = home.month;
        row[`gap_target_month_${p.id}`] = target.month;
      }
    }
  }
  return rows;
...
export function demoData(): Data {
  const properties: Property[] = [
    {
      ...
      owned: true,
      color: colors[0],
      road_address: "서울특별시 마포구 마포대로 195",
    },
    {
      id: "p1",
      name: "공덕자이",
    ...
    },
    {
      ...
      owned: false,
      color: colors[2],
      road_address: "서울특별시 마포구 대흥로 175",
    },
    {
      id: "p3",
      name: "래미안웰스트림",
      ...
      owned: false,
      color: colors[3],
      road_address: "서울특별시 마포구 독막로 246",
    },
  ];
  ...
  for (let i = 35; i >= 0; i--) {
    ...
    if (d > now) d.setUTCDate(1);
    properties.forEach((p, j) => {
      const price =
        Math.round(
          (135000 + j * 18000 + (35 - i) * (850 + j * 90) + Math.sin(i * 0.6 + j) * 3500) / 100,
        ) * 100;
      for (const kind of supportedKinds)
        records.push({
          ...
          price: price + (kind === "estimate" ? 2000 : 0),
          kind,
          source: "예시 데이터",
          note: "화면 체험을 위한 가상 가격입니다.",
        });
    });
  }
...
export function chartAvailability(
  data: Data,
  rows: ReturnType<typeof series>,
  baseId: string,
  kind: Kind,
  baseKind: Kind,
  chart: "gap" | "price",
  selectedComparisonPropertyIds?: string[],
) {
  const hasValue = (id: string) =>
    rows.some((row) => typeof row[id] === "number" && Number.isFinite(row[id]));
  ...
  if (!data.records.length)
    return "아직 가격 기록이 없습니다. 실거래를 불러오거나 가격을 직접 기록해 주세요.";
  if (chart === "gap" && !hasValue(baseId))
    return `선택한 기간에 기준 부동산의 ${labels[baseKind]} 기록이 없습니다. 기준 가격 종류를 바꾸거나 기록을 추가해 주세요.`;
  if (!data.properties.some((p) => p.id !== baseId)) return "비교할 관심단지를 추가해 주세요.";
  if (
    !data.records.some(
      (r) => data.properties.some((p) => p.id === r.property_id && !p.owned) && r.kind === kind,
    )
  )
    return `관심단지의 ${labels[kind]} 기록이 없습니다. 실거래를 먼저 불러와 주세요.`;
  return chart === "gap"
    ? "선택 기간에 보유 주택이나 관심단지의 가격 변화가 없습니다. 조회 기간을 늘리거나 가격 기록을 추가해 주세요."
    : "선택한 기간에 표시할 가격이 없습니다. 조회 기간이나 가격 기준을 바꿔 주세요.";
}
