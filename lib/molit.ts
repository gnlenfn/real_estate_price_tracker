...
export async function fetchMolitResponse(
  url: URL,
  ...
  throw new Error(
    `국토교통부에 연결하지 못했습니다. 잠시 후 다시 시도해 주세요.${lastStatus ? ` (HTTP ${lastStatus})` : lastError ? " (network error)" : ""}`,
  );
}
export function parsePage(xml: string): { items: Item[]; total: number } {
  ...
  if (!response?.header || !["000", "00", "0"].includes(String(response.header.resultCode)))
    throw new Error("국토교통부 응답 오류: API 키의 승인 상태와 호출 한도를 확인해 주세요.");
...
const cleanName = (s: string) =>
  clean(s)
    ...
    .replace(/아파트$/, "");
const active = (i: Item) =>
  !["O", "Y"].includes(String(i.cdealType).toUpperCase()) && !String(i.cdealDay ?? "").trim();
const addressJibun = (address: string) =>
  String(address ?? "")
    .trim()
    .match(/(\d+(?:-\d+)?)$/)?.[1] ?? "";
...
function apartmentItems(items: Item[], lookup: ApartmentLookup) {
  ...
  if (lookup.aptSeq) {
    const exact = rows.filter((i) => clean(i.aptSeq) === clean(lookup.aptSeq!));
    if (exact.length) return exact;
  }
  const jibun = addressJibun(lookup.jibunAddress ?? "");
  if (jibun) {
    const exact = rows.filter((i) => clean(i.jibun) === clean(jibun));
    if (exact.length) return exact;
  }
  const name = cleanName(lookup.name),
    exact = rows.filter((i) => cleanName(i.aptNm) === name);
  if (exact.length) return exact;
  if (name.length >= 4)
    return rows.filter((i) => {
      const candidate = cleanName(i.aptNm);
...
export function apartmentIdentity(items: Item[], lookup: ApartmentLookup) {
  const matches = apartmentItems(items, lookup),
    ids = [...new Set(matches.map((i) => String(i.aptSeq ?? "").trim()).filter(Boolean))];
  const names = [...new Set(matches.map((i) => String(i.aptNm ?? "").trim()).filter(Boolean))];
  return {
    areas: [
      ...new Set(
        matches
          .map((i) => Number(i.excluUseAr))
          .map(areaGroup)
          .filter((n) => Number.isFinite(n) && n > 0),
      ),
    ].sort((a, b) => a - b),
    aptSeq: ids.length === 1 ? ids[0] : null,
    officialName: names.length === 1 ? names[0] : null,
  };
}
export function matchTrades(items: Item[], property: Property, month: string) {
  return apartmentItems(items, {
    name: property.name,
    dong: property.dong,
    aptSeq: property.apt_seq,
    jibunAddress: property.jibun_address,
  })
    .filter((i) => areaGroup(Number(i.excluUseAr)) === areaGroup(property.area))
    .map((i) => ({
      date: `${i.dealYear}-${String(i.dealMonth).padStart(2, "0")}-${String(i.dealDay).padStart(2, "0")}`,
      price: Number(String(i.dealAmount).replace(/,/g, "")),
      note: `전용 ${i.excluUseAr}㎡ · ${i.floor}층`,
    }))
    .filter(
      (r) =>
...
export async function fetchDistrictTrades(
  ...
  for (let page = 1; page <= 100; page++) {
    ...
    const res = await fetchMolitResponse(
      url,
...
export function availableAreas(
  items: Item[],
  name: string,
  dong: string,
  aptSeq?: string | null,
