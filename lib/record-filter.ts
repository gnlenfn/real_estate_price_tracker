...
export type RecordFilter={kind:Kind;propertyId:string;query:string;years:number|null;regionId?:string;now?:Date};
...
export function filterRecords(data:Data,filter:RecordFilter):PriceRecord[]{
 ...
 return data.records.filter(record=>{
  ...
  if(filter.regionId&&filter.regionId!=='all'&&properties.get(record.property_id)?.district!==filter.regionId)return false;
  if(cutoff&&record.date<cutoff)return false;
  if(!query)return true;
  return `${properties.get(record.property_id)?.name||''} ${record.source} ${record.note}`.toLocaleLowerCase('ko-KR').includes(query);
