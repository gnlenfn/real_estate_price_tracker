...
export async function supportServer(request:Request){
 ...
 const auth=createClient(url,anon,{db:{schema:'app'},global:{headers:{Authorization:authorization}},auth:{persistSession:false,autoRefreshToken:false}});
 const {data:{user},error}=await auth.auth.getUser(authorization.slice(7));
 if(error||!user)throw new Error('로그인을 다시 해 주세요.');
 const admin=createClient(url,secret,{db:{schema:'app'},auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}});
 return {user,auth,admin};
}

export async function isSupportAdmin(admin:Awaited<ReturnType<typeof supportServer>>['admin'],userId:string){
 const {data,error}=await admin.schema('admin').from('admins').select('user_id').eq('user_id',userId).maybeSingle();
