...
export function publicTicketResponse(
  ticketId: string,
  _github: { number: number; url: string } | null,
) {
  return { ticketId };
}
...
const oneLine = (value: string | undefined, max: number) =>
  redact(value || "확인되지 않음")
    .replace(/[\r\n]+/g, " ")
    .trim()
    .slice(0, max);
