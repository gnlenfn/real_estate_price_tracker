...
export async function runWeeklyTradeSync(
  trigger: "cron" | "manual",
  options?: { retryOf?: string; actorId?: string },
) {
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL,
    secret = process.env.SUPABASE_SECRET_KEY,
    molitKey = process.env.MOLIT_API_KEY;
  if (!url || !secret || !molitKey) throw new Error("Weekly sync configuration is incomplete");
  const db = createClient(url, secret, {
      db: { schema: "app" },
      auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false },
    }),
    adminDb = db.schema("admin");
  ...
  await adminDb
    ...
    .update({
      ...
      failure_count: 1,
      failures: [{ name: "자동수집", month: "", propertyId: "" }],
    })
    .eq("status", "running")
    .lt("started_at", staleBefore);
  let targets: ReturnType<typeof retryTargets> | undefined,
    failureScope: "targets" | "run" = "targets";
  if (options?.retryOf) {
    const { data: original, error } = await adminDb
      .from("trade_sync_runs")
      .select("id,status,failures,failure_scope")
      .eq("id", options.retryOf)
      .maybeSingle();
  ...
  const { data: run, error: runError } = await adminDb
    ...
    .select("id,request_id,started_at")
    .single();
  if (runError) {
    if (runError.code === "23505") throw new SyncAlreadyRunningError("Sync is already running");
    throw runError;
  }
  await appendAdminEvent(db, {
  ...
  try {
    ...
    for (let offset = 0; ; offset += 1000) {
      const { data, error } = await db
        .from("properties")
        .select(
          "id,user_id,name,district,dong,area,owned,color,apt_seq,kakao_place_id,road_address,jibun_address",
        )
        .order("id")
    ...
    const selectedProperties = targetIds
      ? allProperties.filter((property) => targetIds.has(property.id))
      : allProperties;
    const months = targets
      ? [...new Set(targets.map((target) => target.month))]
      : recentKoreaMonths();
    const allowedPairs = targets
      ? new Set(targets.map((target) => `${target.propertyId}:${target.month}`))
      : undefined;
    const skippedCount = targets
      ? new Set(
          targets
            .filter(
              (target) => !selectedProperties.some((property) => property.id === target.propertyId),
            )
    ...
    const summary = await runScheduledSync(
      ...
      async (property, month, rows) => {
        const { error } = await db.rpc("replace_molit_month_scheduled", {
          p_id: property.id,
    ...
    const total = targets
        ? targets.length - skippedCount
        : selectedProperties.length * months.length,
      status =
    ...
    const { error: updateError } = await adminDb
      ...
      .update({
        ...
        saved_records: summary.savedRecords,
    ...
    if (summary.failures.length)
      logServerError(
        ...
        new Error(
          summary.failures
            .map((item) => `${item.propertyId}/${item.month}: ${item.message}`)
            .join("; "),
        ),
        { runId: run.id, failureCount: summary.failures.length },
      );
    await appendAdminEvent(db, {
      actorId: options?.actorId || null,
      action: "sync.finish",
