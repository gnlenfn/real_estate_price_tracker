...
export async function runScheduledSync(
  ...
  saveMonth: (
    ...
    month: string,
    rows: ReturnType<typeof matchTrades>,
  ) => Promise<void>,
  ...
  const groups = new Map<
    string,
    { district: string; month: string; properties: ScheduledProperty[] }
  >();
  ...
  async function worker() {
    while (next < jobs.length) {
      ...
      try {
        items = await loadDistrict(job.district, job.month);
      } catch (error) {
      ...
      for (const property of job.properties) {
        try {
          const rows = matchTrades(items, property, job.month);
          await saveMonth(property, job.month, rows);
