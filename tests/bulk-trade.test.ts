...
test('bulk selection includes every visible owned and interest property',()=>{
 assert.deepEqual(selectableBulkProperties(properties,'11440').map(property=>property.id),['a','b','home']);
 assert.deepEqual(selectableBulkProperties(properties,'all').map(property=>property.id),['a','b','c','home']);
});
...
test('bulk workload rejects excessive property-month combinations',()=>{
 ...
 assert.throws(
  ()=>assertBulkTradeWorkload(properties.slice(1,3),Array(MAX_BULK_TRADE_JOBS / 2 + 1).fill('2026-09')),
  /한 번에 조회할 수 있는 범위/,
 );
});

test('bulk reload sends every selected property-month pair before refresh',async()=>{
 const events:string[]=[],selected=properties.slice(1,3),months=['2026-08','2026-09'];
 const results=await executeBulkTradeReload({
  properties:selected,
  months,
  request:async(id,month)=>{events.push(`${id}:${month}`);return 1;},
  progress:()=>{},
  active:()=>true,
  refresh:async()=>{events.push('refresh');},
 });
 assert.deepEqual(new Set(events.slice(0,-1)),new Set(['b:2026-08','b:2026-09','a:2026-08','a:2026-09']));
 assert.equal(events.at(-1),'refresh');
 assert.equal(results.reduce((count,result)=>count+result.count,0),4);
});
