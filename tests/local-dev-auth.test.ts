...
test('local development login reuses a server-managed account',()=>{
 const page=source('app/auth/page.tsx'),route=source('app/api/auth/local/route.ts'),adminPage=source('app/admin/login/page.tsx'),adminRoute=source('app/api/auth/local-admin/route.ts');
 ...
 assert.match(adminRoute,/role:'super_admin'/);
